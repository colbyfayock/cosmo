...
export async function getToken() {
    try {
        return execSync('aws ssm get-parameter --name "/nanobot/TELEGRAM_TOKEN" --with-decryption --query Parameter.Value --output text', { encoding: 'utf8' }).trim();
    }
    catch {
...
export async function sendPhoto(options) {
    const token = await getToken();
...
export async function sendMessageWithFallback(chatId, text, photoUrl, parseMode = 'Markdown') {
    if (photoUrl) {
        try {
